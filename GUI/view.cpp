...
void View::drawTable() {
    ...
    ui->tableWidget->setHorizontalHeaderLabels({"управляющий частотой",
                                                "производительность [инст. / цикл]",
                                                "энергопотребление [Джоуль / сек.]",
    ...
    for (int i = 0; i < m_govs.size(); i++) {
        ...
        ui->tableWidget->setItem(i, 3, new QTableWidgetItem(QString::fromStdString(std::to_string(0))));
